from os.path import join
from utils import csv_reader


def get_files(folds_dir, split, fold):
    splits = ['train', 'valid', 'test']
    csv_dir = join(folds_dir, 'split_{}'.format(split), 'fold_{}'.format(fold))
    csv_files = [join(csv_dir,  '{}_s_{}_f_{}.csv'.format(s, split, fold)) for s in splits]
    split_files = [csv_reader(csv) for csv in csv_files]
    return split_files


_classes = {
    2: {'Normal': 0, 'Benign': 0, 'InSitu': 1, 'Invasive': 1},
    4: {'Normal': 0, 'Benign': 1, 'InSitu': 2, 'Invasive': 3},
}


def decode_classes(files: list, num_classes=4) -> list:
    classes = _classes[num_classes]
    files_decoded_classes = []
    for f in files:
        class_name = f[0].split('/')[2]
        files_decoded_classes.append((f[0], classes[class_name]))

    return files_decoded_classes
