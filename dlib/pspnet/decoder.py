...
class PSPBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, pool_size,
        ...
        super().__init__()
        if pool_size == 1:
            use_bathcnorm = False  # PyTorch does not support BatchNorm for 1x1
            # shape
        self.pool = nn.Sequential(
            nn.AdaptiveAvgPool2d(output_size=(pool_size, pool_size)),
            modules.Conv2dReLU(in_channels, out_channels, (1, 1),
...
class PSPModule(nn.Module):
    def __init__(self, in_channels, sizes=(1, 2, 3, 6), use_bathcnorm=True):
        ...
        self.blocks = nn.ModuleList([
            PSPBlock(in_channels, in_channels // len(sizes), size,
