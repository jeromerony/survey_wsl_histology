...
class PAB(nn.Module):
    def __init__(self, in_channels, out_channels, pab_channels=64):
        ...
        self.bottom_conv = nn.Conv2d(in_channels, in_channels, kernel_size=3,
                                     padding=1)
        self.map_softmax = nn.Softmax(dim=1)
        self.out_conv = nn.Conv2d(in_channels, in_channels, kernel_size=3,
                                  padding=1)
...
class MFAB(nn.Module):
    def __init__(self, in_channels, skip_channels, out_channels,
        ...
        # MFAB is just a modified version of SE-blocks, one for skip, one for
        # input
        ...
        self.conv1 = md.Conv2dReLU(
            skip_channels + skip_channels,  # we transform C-prime form high
            # level to C from skip connection
            out_channels,
            kernel_size=3,
            padding=1,
            use_batchnorm=use_batchnorm,
...
class MAnetDecoder(nn.Module):
    def __init__(
        ...
        if n_blocks != len(decoder_channels):
            raise ValueError(
                "Model depth is {}, but you provide `decoder_channels` "
                "for {} blocks.".format(
                    n_blocks, len(decoder_channels)
                )
            )

        encoder_channels = encoder_channels[1:]  # remove first skip with same
        # spatial resolution
        encoder_channels = encoder_channels[::-1]  # reverse channels to start
        # from head of encoder
    ...
    def forward(self, *features):
        ...
        features = features[1:]    # remove first skip with same spatial
        # resolution
        features = features[::-1]  # reverse channels to start from head of
        # encoder
